...
At a low fault rate the agent mostly recovers on its own — it already retries and re-reads error messages. At a high fault rate that stops being enough and success drops. That drop is the point of this project: it's a real number for how much failure this agent can absorb before it breaks.
